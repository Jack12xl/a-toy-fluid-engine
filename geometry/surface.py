...
class ImplicitSurface(Surface):
    ...
    @abstractmethod
    def sign_vector_local(self, local_point: Vector) -> Vector:
        pass

    @ti.func
    def signed_distance(self, world_point: Vector) -> Float:
        local_point = self.transform.to_local(world_point)
        # return self.transform.localScale[0] * self.sign_distance_local(local_point)
        # To support 2 dim in 2D
        sdf = self.transform.localScale * self.sign_vector_local(local_point)
        ret = sdf.norm()
        if self.is_inside_local(local_point):
            ret *= -1.0
        return ret
...
class SurfaceToImplict(ImplicitSurface):
    ...
    def sign_vector_local(self, local_point: Vector) -> Vector:
        """
        To get vector from local_point to closest point
        :param local_point:
        :return:
        """
        p = self.surface.closest_point_local(local_point)
        return p - local_point
